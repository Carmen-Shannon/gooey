...
func drawTextInput(hdc uintptr, ti TextInput) {
	x, y := ti.Position()
	w, h := ti.Size()
	...
	wdws.DrawRectangle(hdc, x, y, x+w, y+h, 4)
	wdws.SelectObject(hdc, oldBrush)
	wdws.DeleteObject(brush)

	// Draw border (optional)
	wdws.DrawEdge(hdc, &[4]int32{x, y, x + w, y + h}, wdws.BD_EDGE_SUNKEN, wdws.BF_RECT)

	// Draw selection highlight if any
	text := ti.Value()
	font := wdws.CreateFont(-ti.TextSize(), ti.Font())
	oldFont := wdws.SelectObject(hdc, font)
	defer func() {
	...
	wdws.SetTextColor(hdc, ti.TextColor())
	...
	if selStart != selEnd && selStart >= 0 && selEnd <= int32(len(text)) {
		...
		prefixWidth, _ := wdws.MeasureText(hdc, font, prefix)
		highlightWidth, _ := wdws.MeasureText(hdc, font, highlight)
		...
		oldHighlightBrush := wdws.SelectObject(hdc, highlightBrush)
		wdws.FillRect(hdc, highlightRect, uintptr(highlightBrush))
		wdws.SelectObject(hdc, oldHighlightBrush)
		wdws.DeleteObject(highlightBrush)
	}

	// Draw text
	textRect := [4]int32{x + 4, y + 2, x + w - 4, y + h - 2}
	wdws.DrawText(hdc, text, &textRect, wdws.DT_LEFT|wdws.DT_VCENTER|wdws.DT_SINGLELINE)
	...
	if ti.Focused() && wdws.CT.Visible && selStart == selEnd {
		...
		caretX := x + 4
		if caretPos > 0 {
			caretWidth, _ := wdws.MeasureText(hdc, font, string(runes[:caretPos]))
			caretX += caretWidth
		}
		caretHeight := int32(float32(h) * 0.5)
		caretY := y + (h-caretHeight)/2
		wdws.DrawRectangle(hdc, caretX, caretY, caretX+2, caretY+caretHeight, 0)
	}
}
